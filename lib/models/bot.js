...
var Bot = orm.Model.extend({
  ...
  // Récupère le temps de cooldown pour une action donnée
  getCooldown: function(actionName) {
    ...
    return actionLogic.cooldown * COOLDOWN_MULTIPLIER;
  ...
  inCooldown: function(actionName) {
    ...
    var self = this;
    var absCooldown = this.getCooldown(actionName);
  ...
  execute: function(actionName, params) {
    ...
    if(!actionLogic) {
      return Promise.reject(new errors.UnknownAction(actionName))
    }
    ...
    return this.inCooldown(actionName)
      ...
      .then(function(results) {
        return {
          action: actionName,
          cooldown: this.getCooldown(actionName),
          results: results
        }
      });
  },
